...
// ----------------------------------------------------------------- //
{
	int 			cpt, cpt2, getLoop=0, dClassicTimeout=1000;
	...
	if(MacVersion >=0x1000){//If OSX
		PLstrcpy(gLatestMessage, "\pLatest Message : Extracting File.  Please wait...");
		tb_WaitTick(180);
		...
		do
		{
			PLstrcpy(gLatestMessage, "\pLatest Message : Extracting File.  Please wait...");//Keep it live so the extractor screen doesn't make the updater white out.
			up_Idle();
			if(TickCount()>gTimeout+dClassicTimeout){
				getLoop=100;
			}
		}
		while(getLoop!=100 && gAbort==false);
		up_InstallFile(which);
		}
	}
#ifdef _FUNCTIONTRACEDEBUGGING
